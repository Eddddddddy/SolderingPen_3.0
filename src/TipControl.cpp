...
double CalculateTemp(double ADC, double P[]) {
  TipTemperature =
      P[0] + ADC * P[1] + ADC * ADC * P[2] + ADC * ADC * ADC * P[3];
...
void PWMOutput(uint8_t pwm) {
  ...
  if (PWMOutput_Lock || ShutdownEvent || Menu_System_State || ERROREvent) {
    ...
    // Menu_System_State=%d ERROREvent=%d\n", PWMOutput_Lock, ShutdownEvent,
    // Menu_System_State, ERROREvent);
    if (MyMOS == PMOS)
      pwm = 255;
    else
      pwm = 0;
...
uint32_t ADCSamplingTime = 0;  // 上次采样时间
uint32_t ADCReadCoolTimer = 0;
...
void TemperatureControlLoop(void) {
  ...
  if (ADC != -1) {
    ...
    if (TempGap > 150)
      ADC_PID_Cycle = ADC_PID_Cycle_List[0];
    else if (TempGap > 50)
      ADC_PID_Cycle = ADC_PID_Cycle_List[1];
    else
      ADC_PID_Cycle = ADC_PID_Cycle_List[2];
    ...
    if (PIDMode) {
      ...
      // 模糊模式
      if (TipTemperature < PID_Setpoint)
        PID_Output = 255;
      else
        PID_Output = 0;
    }
