...
long Calibration_Base[FixNum] = {150, 200, 250, 300, 325,
                                 350, 375, 400, 425, 450};
...
void DrawTempCurve(void) {
  ...
  while (!sys_KeyProcess()) {
    ...
    Disp.setDrawColor(2);
    for (int y = 0; y < 64; y++) {
      x = map(CalculateTemp(y * 64, PTemp), 0, CalculateTemp(4095, PTemp) + 1,
              0, 127);
...
void CalibrationTemperature(void) {
  ...
  for (uint8_t i = 0; i < FixNum;) {
    ...
    sprintf(buffer, "目标 %ld|%.0lf°C", Calibration_Base[i],
            Calibration_Base[i] - GetNTCTemp());
    ...
    uint8_t x;
    for (int y = 0; y < 64; y++) {
      x = map(CalculateTemp(y * 64, TmpP), 0, CalculateTemp(4095, TmpP) + 1, 0,
              127);
      Disp.drawPixel(x, 63 - y);
    }
    Disp.setDrawColor(1);
...
//********** 曲线拟合程序 **********
// 曲线拟合算法来至https://blog.csdn.net/m0_37362454/article/details/82456616
// by欧阳小俊
