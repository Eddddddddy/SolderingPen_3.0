...
static void Write_RButton_FIFO(uint8_t State)
{
    ...
    RButton_FIFO_pwrite++;
    // 按键缓冲区数据大小+1
    if (RButton_FIFO_BufferSize < RButton_FIFO_Size)
...
void sys_PN_click(Button2 &b)
{
    ...
    // 若编码器被锁定，则不允许数值操作
    if (Counter_LOCK_Flag == false)
    {
        if (b.getID() == 1)
        {
            Count = constrain(Count + step, Count_min, Count_max);
        }
        else if (b.getID() == 2)
        {
            Count = constrain(Count - step, Count_min, Count_max);
