...
void TimerEventLoop(void) {
  ...
  // Minute2Millis(ShutdownTime), Minute2Millis(SleepTime));
}
...
void SYS_StateCode_Update(void) {
  ...
  if (TempGap <= 6 && TempCTRL_Status != TEMP_STATUS_ERROR) {
    // 温差接近目标值：正常
    if (!TempToneFlag &&
        millis() - TempToneStabilitytimer > TempToneStabilitytime) {
      // printf("尝试播放到温音效");
      SetSound(Beep3);
      TempToneFlag = true;
    }
    ...
    // PID_Setpoint - 10, TempCTRL_Status != TEMP_STATUS_ERROR, millis() -
    // TempToneStabilitytimer > TempToneStabilitytime);
  ...
  if (UndervoltageAlert != 0 && SYS_Voltage < UndervoltageAlert)
    ...
    UnderVoltageEvent = false;
}
/***
 * @description:
 * 关机事件安全警告画面；重要提示：为了确保您的安全，不建议禁用此功能！
 * @param {*}
 * @return {*}
 */
